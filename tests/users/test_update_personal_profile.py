...
def test_update_own_personal_profile_attribute(attribute, old_value, new_value, reset_github_profile_attributes):
    """
    Test that each att
    ...
    with allure.step("Check the initial profile attributes"):
        ...
        assert getattr(user_profile, attribute) == old_value, f"Expected {attribute} to be present"
...
def test_update_own_personal_profile_not_modified(reset_github_profile_attributes):
    ...
    with allure.step("Retrieve and store ETag from response headers"):
        ...
        allure.attach(etag, name="Received ETag", attachment_type=allure.attachment_type.TEXT)

    with allure.step("Send PATCH request with same data and If-None-Match header to check for 304"):
    ...
    with allure.step("Verify that response status is 304 Not Modified"):
        assert response.status_code == 304, f"Expected 304, but got {response.status_code}"