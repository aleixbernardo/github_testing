...
@pytest.mark.feature('Get Personal Repositories')
class TestPersonalRepositories:
    @pytest.mark.story('Get Personal Repositories')
    @pytest.mark.smoke
    def test_get_personal_repositories(self):
        with allure.step(f"Send GET request to fetch all repositories from logged user'"):
    ...
    @pytest.mark.story('Get Personal Repositories without authorization')
    @pytest.mark.security
    def test_get_personal_repositories_of_logged_user_with_and_without_authorization(self):
        with allure.step(
            f"Send GET request to get the personal repositories without including token"
        ):
        ...
        with allure.step(
            f"Send GET request to get the personal repositories with random token"
        ):
            response = get_repositories_from_logged_user(
                include_token=True, random_token=True
            )
    ...
    @pytest.mark.story('Get Personal Repositories Paginated')
    @pytest.mark.parametrize("page, per_page, expected_count", test_cases)
    def test_get_personal_repositories_of_logged_user_of_user_pagination(
        self, page, per_page, expected_count
    ):
        """
    ...
    @pytest.mark.story('Get Personal Repositories Paginated and Sorted')
    @pytest.mark.parametrize("sort, direction", test_sorting_cases)
    def test_get_personal_repositories_of_logged_user_of_user_sorting(self, sort, direction):
        """
        aleix bernardo contains 9 repos
        """
        with allure.step(
    ...
    @pytest.mark.story('Get Personal Repositories filtering by type')
    @pytest.mark.parametrize("type_value, description", test_filter_cases)
    def test_get_personal_repositories_with_type_filter(self, type_value, description):
        """
    ...
    @pytest.mark.story('Get Personal Repositories filtering by visibility')
    @pytest.mark.parametrize("visibility_value, description", test_filter_cases)
    def test_get_personal_repositories_with_visibility_filter(
        self, visibility_value, description
    ):
        """
        Test filtering repositories by 'type' query parameter.
        from the 9 projects, 4 is member of mberanrdo95
        """
        with allure.step(
            f"Send GET request to fetch repositories of visibility: {visibility_value}"
        ):
            response = get_repositories_from_logged_user(
                visibility=visibility_value, type=None, per_page=50
            )
    ...
    @pytest.mark.story('Get Personal Repositories filtering by affiliation')
    @pytest.mark.parametrize("affiliation_value, description", test_filter_cases)
    def test_get_personal_repositories_with_affiliation_filter(
        self, affiliation_value, description
    ):
        """
        Test filtering repositories by 'type' query parameter.
        from the 9 projects, 4 is member of mberanrdo95
        """
        with allure.step(
            f"Send GET request to fetch repositories of affiliation_value: {affiliation_value}"
        ):
            response = get_repositories_from_logged_user(
                affiliation=affiliation_value, type=None, per_page=50
            )
    ...
    @pytest.mark.story('Get Personal Repositories filtering by type and affiliation')
    def test_combination_type_and_affiliation(self):
        with allure.step(
            f"Send GET request to fetch repositories of affiliation owner and type public"
        ):
            response = get_repositories_from_logged_user(
                affiliation="owner", type="public", per_page=50
            )
    ...
    @pytest.mark.story('Get Personal Repositories filtering by type')
    def test_combination_type_and_visibility_2(self):
        with allure.step(
            f"Send GET request to fetch repositories of affiliation owner and type public"
        ):
            response = get_repositories_from_logged_user(
                visibility="private", type="public", per_page=50
            )
    ...
    @pytest.mark.story('Get Personal Repositories filtering by since')
    @pytest.mark.parametrize("since_value, number_repos", test_filter_cases)
    def test_get_personal_repositories_since_filter(self, since_value, number_repos):
        """
        ...
        with allure.step("Check all dates are after the since value"):
            ...
            for repo in repo_objects:
                ...
                assert (
                    repo_updated_dt > since_value_dt
                ), f"Repo updated_at {repo.updated_at} is not after {since_value}"
    ...
    @pytest.mark.story('Get Personal Repositories filtering by before')
    @pytest.mark.parametrize("before_value, number_repos", test_filter_cases)
    def test_get_personal_repositories_before_filter(self, before_value, number_repos):
        """
        The updated dates for the repos are:
           2022-05-01T20:20:58Z
           2022-09-20T14:20:15Z
        ...
        with allure.step("Check all dates are after the since value"):
            ...
            for repo in repo_objects:
                repo_updated_dt = datetime.strptime(repo.updated_at, "%Y-%m-%dT%H:%M:%SZ")
                assert (
                    repo_updated_dt < before_value_dt
                ), f"Repo updated_at {repo.updated_at} is not before {before_value}"

    @pytest.mark.story('Get Personal Repositories not modified response')
    def test_get_personal_repositories_not_modified_etag(self):
        ...
        with allure.step("Send initial request to GitHub API to get repositories"):
        ...
        with allure.step(
            "Send a second request with If-None-Match header to validate caching"
        ):
        ...
        with allure.step("Verify that response status is 304 Not Modified"):
            ...
            ), f"Expected 304 Not Modified, but got {response.status_code}"
