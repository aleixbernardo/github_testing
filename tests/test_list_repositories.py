...
@pytest.mark.smoke
def test_get_public_repositories_of_user(username):
...
def test_get_public_repositories_of_user_with_and_without_authorization(username):
    ...
    with allure.step(f"Send GET request to fetch user profile for '{username}' without authorization"):
...
@pytest.mark.security
def test_get_public_repositories_of_user_with_and_without_authorization():
    ...
    with allure.step("Validate JSON schema of the response, that contains one more field called notification email"):
        data = response.json()
...
def test_get_public_repositories_of_user_only_1_repo():
    ...
    with allure.step("Convert JSON response to list of Repo object and verify data"):
        repo_object = Repository(**data[0])
        allure.attach(str(repo_object), name="UserProfile Object", attachment_type=allure.attachment_type.TEXT)
...
def test_get_public_repositories_of_user_octocat_pagination(page, per_page, expected_count):
    """
...
def test_get_public_repositories_of_user_octocat_sorting(sort, direction):
    ...
    with allure.step(f"Check that repositories are sorted by {sort} in {direction} order"):
        ...
        sorted_repo_objects = sorted(
            repo_objects,
            key=lambda x: getattr(x, field_ordering).lower() if isinstance(getattr(x, field_ordering), str) else getattr(x, field_ordering),
